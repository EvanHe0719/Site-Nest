...
0.5.7 当前审计目录包含 89 个可执行动作，并覆盖 42 个标记为关键的控件；范围包括站点、会话、页签、页签组、搜索、日程、习惯、时间线、内容标签、Google、Zoho、下载、打印、通知、翻译、自动化、网页脚本和设置。后续新增关键控件时必须同时更新动作目录、`data-action-id` 和测试；未完成能力必须标记为 preview，或禁用并填写原因。
